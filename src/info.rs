...
use pdf::{
    ...
    file::File,
    object::{
        Catalog, GraphicsStateParameters, MaybeRef, NameDictionary, PageRc, PlainRef, Resolve,
        Stream,
    },
...
pub fn print_single_prim(level: usize, resolver: &impl Resolve, name: &Name, prim: &Primitive) {
    ...
    match prim {
        Primitive::Array(vec) => {
            println!("{{");
            vec.iter()
                .for_each(|p| print_single_prim(level + 1, resolver, name, p));
            println!("{}}}", tabs(level));
        }
...
pub fn print_graphicsstate(name: &Name, state: &GraphicsStateParameters, level: usize) {
    ...
    print_if(&state.rendering_intent, level, "rendering_intent");
    ...
    match res {
        Ok(prim) => match prim {
            Primitive::Stream(stream) => {
                ...
                println!("{}Stream {{", tabs(level));
                println!("{}hasFile: {}", tabs(level + 1), &str.file.is_some());

                for filter in &str.filters {
...
pub fn print_page(page: &PageRc, resolver: &impl Resolve) {
    ...
    if let Some(contents) = &page.contents {
        ...
        stats
            .iter()
            .for_each(|(op, val)| println!("{}{} : {}", tabs(2), op, val));
    ...
    if let Some(resources) = &page.resources {
        ...
        }
        {
            ...
            resources.graphics_states.iter().for_each(|(name, state)| {
                print_graphicsstate(name, state, 3);
            });
            println!("{}}}", tabs(2));
        }
        println!("{}}}", tabs(1));
...
pub fn get_pdf_layer_list(pdf_file: &PDF) -> LayerTree {
    let mut layers: BTreeMap<String, Vec<PlainRef>> = BTreeMap::new();

    pdf_file.pages().for_each(|pagerc| match pagerc {
        Ok(page) => {
            if let Some(resources) = &page.resources {
                resources.properties.iter().for_each(|(name, dict)| {
